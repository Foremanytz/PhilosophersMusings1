...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20 px-4 pt-10 max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-gray-500 flex justify-between">
                <span>預設初始交談次數 (每方)</span>
                <span className="text-amber-700 font-bold">{rounds} 次</span>
              </label>
              <input 
                type="range" 
                min="3" 
            ...
            <div className="flex gap-4">
              <input 
                type="text" 
                placeholder="例如：生命的意義、自由意志、AI 的靈魂..."
                value={topic}
              ...
              <button
                ...
                className={`
                  px-8 py-4 rounded-2xl font-bold text-white shadow-lg transition-all min-w-[140px]
      ...
      {/* Dialogue Area */}
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl mb-8 text-center animate-bounce">
          {error}
        </div>
      )}
        ...
        <section className="space-y-6 animate-in fade-in duration-700">
          <div className="flex items-center justify-center gap-8 mb-10 py-6 border-y border-gray-100">
            <div className="flex flex-col items-center">
              <img src={p1?.avatar} className="w-20 h-20 rounded-full border-4 border-amber-500 shadow-xl mb-2 object-cover" />
              <span className="font-bold serif-font text-lg">{p1?.name}</span>
            </div>
            <div className="text-3xl font-serif text-gray-300 italic select-none">VS</div>
            <div className="flex flex-col items-center">
              <img src={p2?.avatar} className="w-20 h-20 rounded-full border-4 border-amber-200 shadow-xl mb-2 object-cover" />
              <span className="font-bold serif-font text-lg">{p2?.name}</span>
            </div>
          </div>

          <div className="bg-white/40 backdrop-blur-md rounded-3xl p-4 sm:p-8 space-y-8 shadow-inner border border-white/50">
            {dialogue.map((msg, idx) => (
              <div 
                key={idx} 
                className={`flex gap-4 ${msg.role === 'p2' ? 'flex-row-reverse' : ''} animate-in slide-in-from-bottom-4 duration-500`}
              >
                ...
                <div className={`max-w-[85%] sm:max-w-[75%] space-y-1 ${msg.role === 'p2' ? 'text-right' : ''}`}>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-2">
                    {msg.name}
                  </p>
                  <div className={`
                    p-5 rounded-3xl text-gray-800 leading-relaxed shadow-sm text-base sm:text-lg
                    ${msg.role === 'p1' ? 'bg-amber-100 rounded-bl-none text-left' : 'bg-white rounded-tr-none text-left'}
                  `}>
                    {msg.content}
                  </div>
                </div>
            ...
            {/* Loading placeholder for Continuation */}
            {isContinuing && (
              <div className="flex gap-4 animate-pulse">
                <div className="w-12 h-12 bg-gray-200 rounded-full flex-shrink-0"></div>
                <div className="h-20 bg-gray-100 rounded-3xl w-2/3"></div>
              </div>
            )}
            ...
            <div className="pt-8 flex flex-col items-center gap-4">
              <button
                ...
                className="group flex items-center gap-3 px-12 py-4 bg-white border-2 border-amber-600 text-amber-700 rounded-full font-bold hover:bg-amber-600 hover:text-white transition-all shadow-md active:scale-95 disabled:opacity-50"
              >
                {isContinuing ? (
              ...
              <p className="text-xs text-gray-400 italic">點擊按鈕讓兩位哲學家針對話題再進行一輪辯論</p>
            </div>
      ...
      {isGenerating && (
        <div className="space-y-12 animate-pulse mt-10">
           ...
           <div className="flex flex-row-reverse gap-4">
              <div className="w-16 h-16 bg-gray-200 rounded-full"></div>
              <div className="h-32 bg-gray-100 rounded-3xl w-3/4"></div>
           </div>
        </div>
      )}
      ...
      {!dialogue.length && !isGenerating && (
        <div className="mt-20 flex flex-col items-center text-gray-400 opacity-60">
          ...
          <p className="serif-font italic text-xl">「未經審視的生活是不值得過的。」— 蘇格拉底</p>
        </div>
      )}
    </div>
  );
